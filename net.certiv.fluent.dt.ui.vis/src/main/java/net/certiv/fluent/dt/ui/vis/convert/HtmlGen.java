...
public class HtmlGen {
	...
	private String build(Kind kind, String content, String base, IPath pathname) {
		...
		switch (kind) {
			case EXPORT:
				...
				sb.appendln(Resources.fromBundle(pluginId, "html/meta.html"));
				sb.appendln(Resources.fromBundle(pluginId, "html/highlight.html"));
				if (converter.useMathJax()) {
					sb.appendln(Resources.fromBundle(pluginId, "html/mathjax.html"));
				}
			...
			case VIEW:
				String preview = Resources.fromBundle(pluginId, "html/liveview.html");
	...
	private String getSocket() {
		...
		path = Strings.removeLeadingCharacters(path, Chars.SLASH);
		path = Strings.removeTrailingCharacters(path, Chars.SLASH);
