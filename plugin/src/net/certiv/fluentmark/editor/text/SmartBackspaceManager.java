...
public class SmartBackspaceManager {
	...
	public static final class UndoSpec {
		...
		/**
		 * Creates a new spec. A specification consists of a number of <code>TextEdit</code>s that
		 * will be executed when backspace is pressed at <code>triggerOffset</code>. The spec will
		 * be removed when it is executed, or if more than <code>lives</code>
		 * <code>TypingRun</code>s have ended after registering the spec.
		 * <p>
		 * Optionally, a child specification can be registered. After executing the spec, the child
		 * spec will be registered with the manager. This allows to create chains of
		 * <code>UndoSpec</code>s that will be executed upon repeated pressing of backspace.
		 * </p>
		 *
			...
			if (fViewer != null && isBackspace(event)) {
				...
				if (spec != null) {
					try {
						...
						if (spec.child != null) register(spec.child);
	...
	public void register(UndoSpec spec) {
		if (fViewer == null) throw new IllegalStateException();
