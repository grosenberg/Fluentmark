...
public class TextEditor {
	...
	void disposeRanges(StyleRange[] ranges) {
		...
		for (StyleRange rangeToDispose : ranges) {
			...
			if (disposeFg && rangeToDispose.foreground != textForeground && rangeToDispose.foreground != null)
				rangeToDispose.foreground.dispose();
			if (disposeBg && rangeToDispose.background != textBackground && rangeToDispose.background != null)
				rangeToDispose.background.dispose();
			if (disposeStrike && rangeToDispose.strikeoutColor != strikeoutColor
					&& rangeToDispose.strikeoutColor != null)
				rangeToDispose.strikeoutColor.dispose();
	...
	void handleResize(ControlEvent event) {
		...
		statusBar.setBounds(rect.x + statusMargin, rect.y + rect.height - sSize.y - statusMargin,
