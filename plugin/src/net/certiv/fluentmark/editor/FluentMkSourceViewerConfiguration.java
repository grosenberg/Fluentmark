...
public class FluentMkSourceViewerConfiguration extends TextSourceViewerConfiguration {
	...
	public FluentMkSourceViewerConfiguration(IColorManager colorManager, IPreferenceStore store, ITextEditor editor,
			String partitioning) {
		super(store);
	...
	public IContentAssistant getContentAssistant(ISourceViewer sourceViewer) {
		if (templatesProcessor == null) {
			templatesProcessor = new FluentMkTemplateCompletionProcessor(editor, IDocument.DEFAULT_CONTENT_TYPE);
		}
		...
		// assistant.setContentAssistProcessor(new FluentMkCodeCompletionProcessor(),
		...
		assistant.setInformationControlCreator(new IInformationControlCreator() {
			...
			public IInformationControl createInformationControl(Shell parent) {
				return new DefaultInformationControl(parent, FluentMkUI.getAdditionalInfoAffordanceString());
			}
		});
	...
	private void configureTemplateProcessor(ContentAssistant assistant, IPreferenceStore store) {
		FluentMkTemplateCompletionProcessor processor = getTemplateProcessor(assistant);
	...
	private FluentMkTemplateCompletionProcessor getTemplateProcessor(ContentAssistant assistant) {
		IContentAssistProcessor p = assistant.getContentAssistProcessor(IDocument.DEFAULT_CONTENT_TYPE);
		if (p instanceof FluentMkTemplateCompletionProcessor) return (FluentMkTemplateCompletionProcessor) p;
		return null;
	}
	...
	private IDialogSettings getSettings(String sectionName) {
		IDialogSettings settings = FluentMkUI.getDefault().getDialogSettings().getSection(sectionName);
		if (settings == null) settings = FluentMkUI.getDefault().getDialogSettings().addNewSection(sectionName);
		return settings;
	}
