 ...
 * <code>CharSequence</code>. Note that the supplied document is not copied; if the document is modified
 * during the lifetime of a <code>DocumentCharacterIterator</code>, the methods returning document content may
 * not always return the same values. Also, if accessing the document fails with a
 * {@link BadLocationException}, any of <code>CharacterIterator</code> methods as well as <code>charAt</code>
...
public class DocumentCharacterIterator implements CharacterIterator, CharSequence {
	...
	public DocumentCharacterIterator(IDocument document, int first, int last) throws IllegalArgumentException {
		...
		if (last > document.getLength()) throw new IllegalArgumentException();
		fDocument = document;
		fFirst = first;
		fLast = last;
		fIndex = first;
	 ...
	 */
	public char current() {
		if (fIndex >= fFirst && fIndex < fLast) try {
	 ...
	 * {@link CharacterIterator#DONE} if a {@link BadLocationException} was thrown when accessing the backing
	 * document.
	 * </p>
	 * 
