...
public class ScssToken extends CommonToken implements IDslToken {
	...
	public ScssToken(int type, String text, int tabWidth) {
		super(type, text);
		this.tabWidth = tabWidth;
	}

	public ScssToken(Pair<TokenSource, CharStream> source, int type, int channel, int start, int stop, int tabWidth) {
	...
	public String toString() {
		...
		String tname = ScssLexer.VOCABULARY.getDisplayName(type);
		String mname = _mode == 0 ? "default" : ScssLexer.modeNames[_mode];
		String chan = channel == 0 ? "DEFAULT" : ScssLexer.channelNames[channel];
		String mark = "/   ";
		if (bol) mark = "/BOL";
		if (bof) mark = "/BOF";
