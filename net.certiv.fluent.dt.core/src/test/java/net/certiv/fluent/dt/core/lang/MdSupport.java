...
import net.certiv.common.grid.Sheet;
import net.certiv.common.grid.SheetBuilder;
...
import net.certiv.fluent.dt.core.lang.md.MdTokenFactory;
...
public class MdSupport {
	...
	public CommonTokenStream createMdTokenStream(Class<?> cls, String name, boolean fill) {
		String data = FsUtil.loadResource(cls, name).get();
	...
	public String render(String name, CommonTokenStream ts, boolean hidden) {
		...
		mb.append(s.render()).nl();
		return mb.toString();
	}

	private Sheet buildSheet(String name, CommonTokenStream ts, boolean hidden) {
		Sheet s = Sheet.of(name);
		SheetBuilder builder = SheetBuilder.on(s).width(80);
		...
		for (Token t : ts.getTokens()) {
			...
			s.appendRow(num, tok, type, txt, line, pos, chan);
		}
