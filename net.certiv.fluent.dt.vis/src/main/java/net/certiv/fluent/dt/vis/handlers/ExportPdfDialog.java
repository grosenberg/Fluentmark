...
public class ExportPdfDialog extends MessageDialog {
	...
	protected Control createCustomArea(Composite parent) {
		...
		txtTmpl.addModifyListener(e -> {
			String pathname = txtTmpl.getText();
			pathname = pathname.equals(Default) ? Strings.EMPTY : pathname;
	...
	private SelectionListener tmplListener = new SelectionAdapter() {
		...
		public void widgetSelected(SelectionEvent e) {
			...
			base = base.endsWith(Strings.SLASH) ? base : base + Strings.SLASH; // default template base dir
			String name = Strings.EMPTY; // internal default template name
			...
			switch (tmplPath.segmentCount()) {
				...
				case 1:
					...
					name = name.equals(Default) ? Strings.EMPTY : name;
					break;
				default:
