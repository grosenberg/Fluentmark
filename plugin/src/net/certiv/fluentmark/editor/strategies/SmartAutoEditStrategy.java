...
public class SmartAutoEditStrategy implements IAutoEditStrategy {
	...
	/**
	 * Creates a new SmartAutoEditStrategy.
	 *
	...
	public void customizeDocumentCommand(IDocument document, DocumentCommand command) {
		...
		if (isMultilineSelection(document, command)) return;
		...
		try {
			...
			if (manager != null && store.getBoolean(Prefs.EDITOR_SMART_BACKSPACE)) {
				...
				UndoSpec s2 = new UndoSpec(position + insertion.length(),
						new Region(command.offset + command.text.length(), 0), new TextEdit[] { smart, raw }, 2, s1);
				manager.register(s2);
			}
	 ...
	 * @return a <code>String</code> consisting of <code>character</code> plus any additional
	 *         spacing
	 */
	 ...
	 * @param partitioning the document partitioning
	 * @return an insert position relative to the line start if <code>line</code> looks like being
	 *         an array initialization at <code>offset</code>, -1 otherwise
	 */
	 ...
	 * <code>Character.isWhitespace(document.getChar(pos))</code> evaluates to <code>false</code>
	...
	// [<code>position</code>, <code>scanTo</code>) that resides in a Java partition, or
	 ...
	 * @param position the first character position in <code>document</code> to be considered
	 * @param partitioning the document partitioning
	 * @return <code>true</code> if the content of <code>document</code> looks like an anonymous
	 *         class definition, <code>false</code> otherwise
	 */
	private static boolean looksLikeAnonymousClassDef(IDocument document, int position, String partitioning) {
		int previousCommaParenEqual = scanBackward(document, position - 1, partitioning, -1,
				new char[] { ',', '(', '=' });
		if (previousCommaParenEqual == -1 || position < previousCommaParenEqual + 5) // 2 for
																					 // borders, 3
																					 // for "new"
			return false;
	 ...
	 * @param partitioning the document partitioning
	 * @return the position in <code>document</code> of the matching parenthesis, or -1 if none can
	 *         be found
	 */
	private static int findOpeningParenMatch(IDocument document, int position, String partitioning) {
		...
		try {
			...
			int depth = 1;
			while (true) {
				position = scanBackward(document, position - 1, partitioning, -1,
	 ...
	 * made sure that <code>like</code> is preceded by some non-identifier character or stands at
	 * the document start.
	 *
	 * @param document the document being modified
	 * @param position the first character position in <code>document</code> to be considered
	 * @param like the <code>String</code> to look for.
	 * @return <code>true</code> if <code>document</code> contains <code>like</code> such that it
	 *         ends at <code>position</code>, <code>false</code> otherwise
	 */
	...
	/**
	 * From <code>position</code> to the left, eats any whitespace and then a pair of brackets as
	 * used to declare an array return type like
	 * 
	 * <pre>
	 * String [ ]
	 * </pre>
	 * 
	 * . The return value is either the position of the opening bracket or <code>position</code> if
	 * no pair of brackets can be parsed.
	 *
	 * @param document the document being modified
	 * @param position the first character position in <code>document</code> to be considered
	 * @param partitioning the document partitioning
	...
	/**
	 * From <code>position</code> to the left, eats any whitespace and the first identifier,
	 * returning the position of the first identifier character (in normal read order).
	 * <p>
	 * When called on a document with content <code>" some string  "</code> and positionition 13,
	 ...
	 * <code>maxOffset</code>. Comment partitions or empty java partitions do never yield valid
	 * insert positions.
	 *
	 * @param doc the document being modified
