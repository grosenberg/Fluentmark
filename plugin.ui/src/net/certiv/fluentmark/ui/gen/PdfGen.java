...
public class PdfGen {
	 ...
	 */
	public static void save(String base, IDocument doc, Page model, String template, String pathname) {
	...
	protected static File buildGraphs(IDocument doc, Page model) throws IOException {
		File dir = getTmpFolder();

		// evaluate parts representing dot and uml code blocks
		// build latex graphics include blocks
		MultiTextEdit edit = new MultiTextEdit();
		for (PagePart part : model.getPageParts(Item.CODE_BLOCK)) {
			ISourceRange range = part.getSourceRange();

			switch (part.getMeta()) {
				case IPageElement.DOT:
					ISourceRange subRange = calcDotRange(model, range);
					...
					if (ok) edit.addChild(splice(doc, model, range, tmpfile));
					break;

				case IPageElement.UML:
	...
	private static ISourceRange calcDotRange(Page model, ISourceRange clipRange) {
		int dotBegLine = clipRange.getBegLine() + 1;
		...
		endOffset += model.getTextLength(dotEndLine) + model.delim().length();
	...
	private static TextEdit splice(IDocument doc, Page model, ISourceRange range, File tmpfile) {
		String dir = tmpfile.getParent().replace("\\", "/") + "/";
		String name = tmpfile.getName();

		String figure = GRAPHIC.replaceAll("\\R", model.delim());
