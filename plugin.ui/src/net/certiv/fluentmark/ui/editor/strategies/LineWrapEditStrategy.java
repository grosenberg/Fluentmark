...
import net.certiv.dsl.ui.editor.text.autoedit.old.AutoEdit2;
...
public class LineWrapEditStrategy implements IAutoEditStrategy {
	...
	public void customizeDocumentCommand(final IDocument doc, final DocumentCommand cmd) {
		...
		try {
			if (TextUtilities.endsWith(doc.getLegalLineDelimiters(), cmd.text) != -1) {
				if (AutoEdit2.isBlankLine(doc, cmd.offset)) {
					int beg = AutoEdit2.getLineOffset(doc, cmd.offset);
	...
	private void autoIndent(IDocument doc, DocumentCommand cmd, boolean forceNL) {
		...
		try {
			int offset = AutoEdit2.getOffset(doc, cmd);
			IRegion info = doc.getLineInformationOfOffset(offset);
			int beg = info.getOffset();
			int end = AutoEdit2.getEndOfWs(doc, beg, cmd.offset); // end of WS
			int lmw = 0;

			PagePart part = editor.getPageModel().partAtOffset(offset);
			if (part != null && part.getItem() == Item.LIST) {
				lmw = AutoEdit2.listMarkerWidth(doc, end);
			}
